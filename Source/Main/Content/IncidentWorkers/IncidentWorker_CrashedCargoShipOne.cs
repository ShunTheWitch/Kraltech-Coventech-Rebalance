...
public class IncidentWorker_CrashedCargoShipOne : IncidentWorker
{
    private const float ShipPointsFactor = 0.9f;

    private const int IncidentMinimumPoints = 9000;

    private const float DefendRadius = 150f;
    ...
    protected override bool TryExecuteWorker(IncidentParms parms)
    {
        ...
        IntVec3 intVec;
        CellFinderLoose.TryFindSkyfallerCell(ThingDefOf.CargoShuttleCrashing, map, out intVec, 14, default, -1, false, true, true, true);
        var thing = ThingMaker.MakeThing(ThingDefOf.CargoShuttleCrashedOne);
        bool result;
        ...
        {
            var points = Mathf.Max(parms.points * 1500f, 9000f);
            ...
            GenSpawn.Spawn(SkyfallerMaker.MakeSkyfaller(ThingDefOf.CargoShuttleCrashing, thing), intVec, map);
