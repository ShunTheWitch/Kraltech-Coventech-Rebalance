...
public class IncidentWorker_DamagedShipChunkDrop : IncidentWorker
{
    ...
    static IncidentWorker_DamagedShipChunkDrop()
    {
    }
    ...
    {
        SkyfallerMaker.SpawnSkyfaller(ThingDefOf.DamagedShipChunkIncoming, ThingDefOf.DamagedShipChunk, pos, map);
    }

    private bool TryFindShipChunkDropCell(IntVec3 nearLoc, Map map, int maxDist, out IntVec3 pos)
    {
        return CellFinderLoose.TryFindSkyfallerCell(ThingDefOf.DamagedShipChunkIncoming, map, out pos, 10, nearLoc, maxDist, false);
    }
}
