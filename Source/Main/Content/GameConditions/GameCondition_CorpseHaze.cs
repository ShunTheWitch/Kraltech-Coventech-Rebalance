...
public class GameCondition_CorpseHaze : GameCondition
{
    ...
    private static readonly SkyColorSet NoxiousHazeColors = new(new ColorInt(150, 75, 0).ToColor, new ColorInt(150, 75, 0).ToColor, new ColorInt(150, 75, 0).ToColor, 0.1f);
    ...
    static GameCondition_CorpseHaze()
    {
    }
    ...
    public static void DoPawnToxicDamage(Pawn p, bool protectedByRoof = true, float extraFactor = 1f)
    {
        if (!p.Spawned || !protectedByRoof || !p.Position.Roofed(p.Map))
        {
            ...
            if (num != 0f)
            {
                ...
                num *= num2;
                HealthUtility.AdjustSeverity(p, HediffDefOf.LungRot, num);
            }
        }
    }
