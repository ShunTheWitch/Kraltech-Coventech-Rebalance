...
public class GameCondition_ExtremeHeatWave : GameCondition
{
    ...
    private readonly SkyColorSet ToxicFalloutColors = new(new ColorInt(216, 25, 0).ToColor, new ColorInt(234, 10, 25).ToColor, new Color(0.6f, 0.8f, 0.5f), 0.85f);
    ...
    public static void DoPawnToxicDamage(Pawn p, bool protectedByRoof = true, float extraFactor = 1f)
    {
        if (!p.Spawned || !protectedByRoof || !p.Position.Roofed(p.Map))
        {
            ...
            num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.ColdNullNaniteResistance), 0f);
            if (ModsConfig.BiotechActive) num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.ColdNullNaniteEnvironmentResistance), 0f);
            num *= extraFactor;
            if (num != 0f)
            {
                var num2 = Mathf.Lerp(0.85f, 1.15f, Rand.ValueSeeded(p.thingIDNumber ^ 74374237));
                num *= num2;
                HealthUtility.AdjustSeverity(p, HediffDefOf.ColdNullNanites, num);
            }
        }
    }
