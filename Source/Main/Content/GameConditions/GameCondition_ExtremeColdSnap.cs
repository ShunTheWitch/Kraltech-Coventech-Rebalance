...
public class GameCondition_ExtremeColdSnap : GameCondition
{
    ...
    {
        new WeatherOverlay_SnowHard()
    };

    private readonly SkyColorSet ToxicFalloutColors = new(new ColorInt(0, 0, 255).ToColor, new ColorInt(234, 200, 255).ToColor, new Color(138f, 43f, 226f), 2f);

    public override int TransitionTicks => 5000;

    public override float TemperatureOffset()
    {
        return GameConditionUtility.LerpInOutValue(this, TransitionTicks, -30f);
    }
    ...
    public static void DoPawnToxicDamage(Pawn p, bool protectedByRoof = true, float extraFactor = 1f)
    {
        if (!p.Spawned || !protectedByRoof || !p.Position.Roofed(p.Map))
        {
            ...
            num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.HeatNullNaniteResistance), 0f);
            if (ModsConfig.BiotechActive) num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.HeatNullNaniteEnvironmentResistance), 0f);
            num *= extraFactor;
            if (num != 0f)
            {
                var num2 = Mathf.Lerp(0.85f, 1.15f, Rand.ValueSeeded(p.thingIDNumber ^ 74374237));
                num *= num2;
                HealthUtility.AdjustSeverity(p, HediffDefOf.HeatNullNanites, num);
            }
        }
    }
