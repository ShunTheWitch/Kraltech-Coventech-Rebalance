...
public class GameCondition_CrimsonRain : GameCondition
{
    ...
    private readonly SkyColorSet ToxicFalloutColors = new(new ColorInt(136, 8, 8).ToColor, new ColorInt(136, 8, 8).ToColor, new Color(0f, 0.89f, 0.28f), 0.1f);
    ...
    public static void DoPawnToxicDamage(Pawn p, bool protectedByRoof = true, float extraFactor = 1f)
    {
        if (!p.Spawned || !protectedByRoof || !p.Position.Roofed(p.Map))
        {
            ...
            num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.RageVirusResistance), 0f);
            if (ModsConfig.BiotechActive) num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.RageVirusEnvironmentResistance), 0f);
            num *= extraFactor;
            if (num != 0f)
            {
                var num2 = Mathf.Lerp(0.85f, 1.15f, Rand.ValueSeeded(p.thingIDNumber ^ 74374237));
                num *= num2;
                HealthUtility.AdjustSeverity(p, HediffDefOf.RageVirus, num);
            }
        }
    }
