...
public class GameCondition_ToxicRainNew : GameCondition
{
    ...
    private const float ToxicPerDay = 1.9f;

    private const float PlantKillChance = 0.0965f;
    ...
    private readonly SkyColorSet ToxicFalloutColors = new(new ColorInt(216, 255, 0).ToColor, new ColorInt(234, 200, 255).ToColor, new Color(0.8f, 0.1f, 0.7f), 0.95f);
    ...
    private void DoPawnsToxicDamage(Map map)
    {
        ...
        for (var i = 0; i < allPawnsSpawned.Count; i++) GameCondition_ToxicRain.DoPawnToxicDamage(allPawnsSpawned[i]);
    }
    ...
    public static void DoPawnToxicDamage(Pawn p, bool protectedByRoof = true, float extraFactor = 1f)
    {
        if (!p.Spawned || !protectedByRoof || !p.Position.Roofed(p.Map))
        {
            ...
            if (ModsConfig.BiotechActive) num *= Mathf.Max(1f - p.GetStatValue(StatDefOf.ToxicEnvironmentResistance), 0f);
            num *= extraFactor;
            if (num != 0f)
            {
                var num2 = Mathf.Lerp(0.85f, 1.15f, Rand.ValueSeeded(p.thingIDNumber ^ 74374237));
                num *= num2;
                HealthUtility.AdjustSeverity(p, HediffDefOf.NeuroToxin, num);
            }
        }
    }
