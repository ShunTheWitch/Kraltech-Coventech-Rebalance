...
public class GameCondition_DisableElectricityAndCauseRadiation : GameCondition
{
    ...
    private const float PlantKillChance = 0.017f;
    ...
    private readonly SkyColorSet ToxicFalloutColors = new(new ColorInt(138, 43, 226).ToColor, new ColorInt(236, 196, 224).ToColor, new Color(138f, 43f, 226f), 2f);
    ...
    public static void DoPawnToxicDamage(Pawn p, bool protectedByRoof = true, float extraFactor = 1f)
    {
        if (!p.Spawned || !protectedByRoof || !p.Position.Roofed(p.Map))
        {
            ...
            num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.RadiationResistance), 0f);
            if (ModsConfig.BiotechActive) num *= Mathf.Max(1f - p.GetStatValue(StatDefOf_NanitesEX.RadiationEnvironmentResistance), 0f);
            num *= extraFactor;
            if (num != 0f)
            {
                var num2 = Mathf.Lerp(0.85f, 1.15f, Rand.ValueSeeded(p.thingIDNumber ^ 74374237));
                num *= num2;
                HealthUtility.AdjustSeverity(p, HediffDefOf.RadiationPoisoning, num);
            }
        }
    }
