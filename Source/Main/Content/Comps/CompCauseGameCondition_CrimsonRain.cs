...
public class CompCauseGameCondition_CrimsonRain : CompCauseGameCondition
{
        ...
        if (Active && Find.TickManager.TicksGame % 3451 == 0)
        {
            ...
            for (var i = 0; i < caravans.Count; i++)
                if (Find.WorldGrid.ApproxDistanceInTiles(caravans[i].Tile, MyTile) < Props.worldRange)
                {
                    ...
                    for (var j = 0; j < pawnsListForReading.Count; j++) GameCondition_CrimsonRain.DoPawnToxicDamage(pawnsListForReading[j]);
                }
        }
    }
}